...
RAW_TOKEN = os.getenv("METAAPI_TOKEN") or ""
...
def test_trade():
    b=base()
    log=f"TOKEN SET: {bool(TOKEN)} LEN: {len(TOKEN)} STARTS: {TOKEN[:15]}...<br>"
    log+=f"Using: {b}<br><br>"
    try:
        r=requests.get(f"{b}/accountInformation", headers=H(), timeout=15, verify=False)
        log+=f"AccountInfo {r.status_code}: {r.text[:1000]}<br><br>"
...
@app.route('/health')
def h(): return f"OK V15.2 token_len={len(TOKEN)} set={bool(TOKEN)}"
