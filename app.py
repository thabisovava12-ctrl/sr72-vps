...
def test_trade():
    ...
    try:
        ...
        log+=f"AccountInfo {r.status_code}: {r.text[:800]}<br><br>"
    ...
    try:
        ...
        log+=f"Current-Price {r.status_code}: {r.text[:800]}<br><br>"
        if r.ok:
            j=r.json()
    ...
    try:
        ...
        r=requests.post(f"{b}/trade", headers=H(), json=payload, timeout=20, verify=False)
        log+=f"SELL {r.status_code}: {r.text}<br><br>"
    except Exception as e: log+=f"SELL ERR {e}<br><br>"
    return log
